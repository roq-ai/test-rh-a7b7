...
function DeliveryRequestListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<DeliveryRequestInterface[]>(
    ...
    () =>
      getDeliveryRequests({
        relations: ['user_delivery_request_customer_idTouser', 'user_delivery_request_courier_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_delivery_request_customer_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_delivery_request_courier_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_delivery_request_customer_idTouser?.id}`}>
                          {record.user_delivery_request_customer_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_delivery_request_courier_idTouser?.id}`}>
                          {record.user_delivery_request_courier_idTouser?.email}
                        </Link>
                      </Td>
                    )}
